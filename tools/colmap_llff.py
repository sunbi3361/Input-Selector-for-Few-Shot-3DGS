...
def pipeline(scene, base_path, n_views, idx):
    llffhold = 8
    view_path = str(n_views) + '_' + str(idx) + '_views'
    ...
    if n_views > 0:
        ...
        idx_sub = random.sample(range(len(train_img_list)), n_views)
...
#for scene in ['flower']:
#for scene in ['fern', 'flower', 'fortress',  'horns']:
